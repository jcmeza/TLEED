...
CGPS      SUBROUTINE GSStleed1(DIR,RANK,PARM,MINB,MAXB,NTYPE,FITVAL)
      SUBROUTINE eval_update(problem_dir,PARM,NTYPE,parm_out,ntype_out)
C      SUBROUTINE GPStleed1(problem_dir,DIR,RANK,PARM,MINB,MAXB,NTYPE,
C     & FITVAL)
      
      PARAMETER (NMAX=14,NSUB=6,NIDEN=5,NDIM=3,PENALTY=1.6)
      
      REAL PARM(NMAX,NDIM),PARM_0(NMAX,NDIM)
C,MINB(NMAX,NDIM),MAXB(NMAX,NDIM)

      real parm_out(NMAX,NDIM)
      integer ntype_out(NMAX),NTYPE_0(NMAX)
...
C      CHARACTER*3 PROCID
C      CHARACTER*3 WORKID
...
C Input substrate coordinates, COORSUB0, AA is the length of unit cell.
     ...
     &0.1,0.1,0.1,0.3,0.3,0.5/
      ...
      TLEED4 = problem_dir // '/tleed4i_out'
      TLEED5 = problem_dir // '/tleed5i_out'
      SEARCHS =problem_dir // '/searchs_out'
      TRACE = problem_dir // '/trace_out' 
...
C	write(0,*) PARM,NTYPE,NCODE,NMAX,SYMC,RESULT,RMIN,RMAX,NIDEN,
C     &     COORSUB,NTYPSUB,NSUB,AA
...
C     &             PARM(I,3)
c     enddo
Cprint out the parameters after the valuate
C           DO I=1,NMAX
C              WRITE (0,"(I4,3F8.4)") NTYPE(I),PARM(I,1),
C     &             PARM(I,2),PARM(I,3)
C           ENDDO
           ...
           DO I=1,NMAX
              ntype_out(I)=NTYPE(I)
              parm_out(I,1)=PARM(I,1)
              parm_out(I,2)=PARM(I,2)
              parm_out(I,3)=PARM(I,3)
           ENDDO

 
           OPEN(UNIT=99,FILE=TRACE,STATUS='OLD',ACCESS='APPEND')
           WRITE (99,*) "ID: ",PROCID," Parms after VALUATE"
           DO I=1,NMAX
              WRITE (99,"(I4,3F8.4)") NTYPE(I),PARM(I,1),
     &             PARM(I,2),PARM(I,3)
           ENDDO
C write into the difference of parms
           WRITE (99,*) "parm difference:" 
           DO I=1,NMAX
              WRITE (99,"(I4,3F8.4)") NTYPE(I)-ntype_0(I),
     &             PARM(I,1)-parm_0(I,1),
     &             PARM(I,2)-parm_0(I,2),PARM(I,3)-parm_0(I,3)
           ENDDO
           CLOSE (UNIT=99)

C           WRITE (0,*) "parm difference:" 
C           DO I=1,NMAX
C              WRITE (0,"(I4,3F8.4)") NTYPE(I)-ntype_0(I),
C     &             PARM(I,1)-parm_0(I,1),
C     &             PARM(I,2)-parm_0(I,2),PARM(I,3)-parm_0(I,3)
C           ENDDO
        END IF
