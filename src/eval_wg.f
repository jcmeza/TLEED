...
C called by it used), otherwise program crashes. 
      SUBROUTINE eval_wg(PARM,NTYPE,parm_out,rntype_out)
...
C      INTEGER NTYPE(NMAX),ntype_out(NMAX)
 ...
 100   enddo          
       ...
       call eval(PARM_f,NTYPE,parm_out_f,ntype_out)
...
C corresponds to input PARM
      mnsize =mxGetM(prhs(1))*mxGetN(prhs(1)) 
...
C     Call the computational routine.
      call eval_wg(in1,in2,out1,rout2)
