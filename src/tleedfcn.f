      ...
      implicit none
c
c     New program derived from runtleed and to be used as
c     as interface between Matlab and TLEED
c     Parameters are read from a new inputfile called tleedinputs.dat and
c     the output function value is to be written to tleedfval.dat

      integer :: nin, nout
      integer :: my_iostat
      character(256) :: my_iomsg
c     
c     kleed/tleed variables
c
      integer nmax, ndim
      ...
      logical success
c
c     data_dir is the directory used by TLEED to get all its other input parameters
...
c     Set up file I/O 
c
      inputfile = trim(problem_dir)//'/tleedinputs.dat'
      open(newunit=nin,file=inputfile,iostat=my_iostat, iomsg=my_iomsg)
      IF(my_iostat /= 0) THEN
         WRITE(*,*) 'Failed to open tleedinputs file'
         WRITE(*,*) ' iomsg='//trim(my_iomsg)
         STOP
      ENDIF
      
      outfile = trim(problem_dir)//'/tleedfval.dat'
      open(newunit=nout,file=outfile,iostat=my_iostat, iomsg=my_iomsg)
      IF(my_iostat /= 0) THEN
         WRITE(*,*) 'Failed to open tleedfval file'
         WRITE(*,*) ' iomsg='//trim(my_iomsg)
         STOP
      ENDIF
