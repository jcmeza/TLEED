   ...
   while ~terminate(Options.Term,RunData,Options.runStochastic)
      ...
      if strcmp(upper(Problem.typeProblem),'TRUTH')
         dispDebug(2,Options.debug, ...
           [Problem.typeProblem,':   New mesh size = ',num2str(RunData.delta)]);
      end
   end
...
% Set up diary file if debugging option is selected
...
case {'DACE','NW','SPS-DACE','SPS-NW','CUSTOMS'}
...
case 'LHS'
   ...
   % Determine LHS grid size and call LHS
   delta   = max(1.0,RunData.delta);
   ngrid   = round(max(2,n)/delta)*ones(n,1);
   z       = lhs(ngrid,Search.nPoints,Options.lhsStrength);

   % Convert LHS numbers to points in the feasible region 
   nPoints = size(z,2);
   S = [];
   spacing = (UB - LB)./(ngrid-1);
   if ~isempty(z)
      for k = 1:nPoints
         S(k).x = LB + spacing.*(z(:,k)-1);
         S(k).p = pcenter.p;
	 if inOmega(S(k),Problem.Omega)
	    disp('Hooray-LHS point is in Omega');
	 else
	    disp('BOOOOOO-LHS point not in Omega');
	 end
      end
...
case 'LHS2'
   ...
   % Recompute Omega and the scaling factors
   Options.removeRedundancy = 0;
   [Problem.Omega,scale] = updateOmega(Problem,Options,pcenter);
%   scale = min(scale,scale/norm(scale,inf));

   % Make sure the mesh is fine enough to generate enough points
   maxrand = 2^5;
   delta = max(1.0,RunData.delta);
   scale = (Problem.Omega.u - Problem.Omega.l)/max(4,pcenter.n);

%   delta = min(RunData.delta,min((Problem.Omega.u - Problem.Omega.l)/10));
   ngrid = 0;
   while ngrid <= Search.nPoints
      [(Problem.Omega.l(1:pcenter.n) - pcenter.x)./(scale*delta), ...
       (Problem.Omega.u(1:pcenter.n) - pcenter.x)./(scale*delta)]
      LB = fix((Problem.Omega.l(1:pcenter.n) - pcenter.x)./(scale*delta));
      UB = fix((Problem.Omega.u(1:pcenter.n) - pcenter.x)./(scale*delta));
      LB(LB < -maxrand) = -maxrand/2;
      UB(UB >  maxrand) =  maxrand/2;
      ngrid = prod(UB-LB+1);
      delta = delta*Options.meshRefine;
   end
   delta = delta/Options.meshRefine;

   % Call the Latin hypercube search function and form trial iterates
   z = lhs(UB-LB+1,Search.nPoints,Options.lhsStrength);
   nPoints = size(z,2);
   S = [];
   if ~isempty(z)
      z = z + repmat(LB,1,size(z,2)) + 1;
      for k = 1:nPoints
         S(k).x = pcenter.x + delta*scale.*z(:,k);
         S(k).p = pcenter.p;
         [LB,z(:,k),UB]
         [Problem.Omega.l,[pcenter.x],[S(k).x],Problem.Omega.u]
         pause
      end
      full = (RunData.nIter == 0);
   end
...
[ignore,success,Filter] = evalPointSet(Problem,S,full,Options,Filter, ...
                                       RunData.RS,2);
return
...
%  smallSize = maximum size of a small sample
%  nSamples  = number points to be sampled for each strength level
...
% Determine sample space dimension and number of points to be evaluated
n = length(nGrid);
nSamples = max(nGrid);
nPoints  = min([prod(nGrid), budget, strength*nSamples]);

% Generate a "small" number of LHS points (randperm is an expensive function)
smallSize = nSamples <= 10000;
if smallSize
   z = zeros(n,min(strength*nSamples,10000));
   for k = 1:n
      for j = 1:strength
         zTemp = [];
         for i = 0:max(ceil((nSamples - nGrid(k))/nSamples),nSamples)
            zTemp = [zTemp, randperm(nGrid(k))];
         end
         z(k,1+nSamples*(j-1):nSamples*j) = zTemp(1:nSamples);
      end
   end
   z(:,nPoints+1:end) = [];
   return
...
%  searchFile     = temporary storage of Search.file
%*******************************************************************************
...
if strncmp(upper(Search.type),'SPS',3) || strcmp(upper(Search.type),'CUSTOMS')
   [ignore,searchFile] = fileparts(Search.file);
   for k = 1:length(Sites)
      if isempty(Sites(k).sf)
         switch nargin(searchFile)
            case {1}
               if nargout(searchFile) == 1
                  Sites(k).sf = feval(searchFile,Sites(k).x);
                  Sites(k).sc = [];
               else
                  [Sites(k).sf, Sites(k).sc] = feval(searchFile,Sites(k).x);
               end
            case {2}
               if nargout(searchFile) == 1
                  Sites(k).sf = feval(searchFile,Cache(k).x,Cache(k).p);
                  Sites(k).sc = [];
               else
                  [Sites(k).sf, Sites(k).sc] = ...
                                feval(searchFile,Cache(k).x,Cache(k).p);
               end
            otherwise
...
surrogate = recalSurrogate(iterate,surrogate,optimizer,Filter, ...
                           str2func(Search.dace.reg), ...
                           str2func(Search.dace.corr), ...
                           Search.dace.theta*ones(n,1), ...
                           Search.dace.lower*ones(n,1), ...
                           Search.dace.upper*ones(n,1));
return
...
%  Filter      = indices into iterate that contain the filter
%  ind         = indices of iterates with a finite objective function value
...
%    Sur.Options             = Sur.Defaults.Options;
%    Sur.Options.delta0      = 4*Options.delta0;
%    Sur.Options.Term.delta  = 10*Options.Term.delta;
...
[P.f,P.c,P.h,P.sf,P.sc,P.sh] = deal([]);
...
if isStochastic
   ...
   if R.RS.sdev ~= 0.0
      term = term && (R.RS.sdev/R.RS.iz >= T.RS.noise);
   end
end
